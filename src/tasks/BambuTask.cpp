...
void handleAssignTray(rtos::RtosContext& ctx, PrinterConnections& connections,
  ...
  // command.trayType/name/manufacturer are the raw Spoolman filament fields
  // (empty trayType clears the slot, see rtos::BambuCommand::trayType) --
  // resolved here to Bambu's own AMS profile (tray_info_idx/canonical
  // tray_type/nozzle_temp_min/max) via the priority-ordered schema-v2 rule
  // table (Nutzerwunsch 2026-08-30, replaces the earlier flat
  // material-only lookup). Deliberately NOT the individual Spoolman
  // filament's bambu_temp_min/bambu_temp_max: those only ever affect the
  // load/unload temperature range shown on the AMS slot, not the actual
  // print temperature (that comes from the slicer/filament profile used
  // for a print job), so Bambu's own generic-material defaults are used
  // instead -- see services::resolveBambuMaterialRule() and
  // docs/bambu-protocol.md.
  ...
  if (command.trayType[0] != '\0') {
    ...
    const services::BambuMaterialResolveInput resolveInput{
        command.trayType, command.name, command.manufacturer};
    const services::BambuMaterialResolveResult resolved =
        services::resolveBambuMaterialRule(*table, resolveInput);
    if (resolved.status == services::BambuMaterialResolveStatus::Ambiguous) {
      FS_LOGW(services::LogComponent::Bambu,
              "[BAMBU] Material resolve failed reason=ambiguous_material_mapping "
              "priority=%ld rules=\"%s\"",
              static_cast<long>(resolved.matchedPriority),
              resolved.ambiguousRuleIds);
      FS_LOGW(services::LogComponent::Bambu,
              "AssignTray rejected printer_id=%u ams_id=%u tray_id=%u "
              "material=\"%s\" reason=ambiguous_material_mapping",
              static_cast<unsigned>(command.printerId),
              static_cast<unsigned>(command.amsId),
              static_cast<unsigned>(command.trayId), command.trayType);
      publishBambuError(
          ctx, command.requestId, command.printerId,
          "Mehrdeutige Material-Zuordnung, bitte bambu_materials.json korrigieren");
      return;
    }
    if (resolved.status == services::BambuMaterialResolveStatus::NoMatch) {
      ...
      FS_LOGW(services::LogComponent::Bambu,
              "[BAMBU] Material resolve failed material=\"%s\" reason=no_match",
              command.trayType);
      FS_LOGW(services::LogComponent::Bambu,
              "AssignTray rejected printer_id=%u ams_id=%u tray_id=%u "
    ...
    if (resolved.status == services::BambuMaterialResolveStatus::Unsupported) {
      FS_LOGW(services::LogComponent::Bambu,
              "[BAMBU] Material unsupported material=\"%s\" reason=\"%s\"",
              command.trayType, resolved.rule->result.reason);
      FS_LOGW(services::LogComponent::Bambu,
              "AssignTray rejected printer_id=%u ams_id=%u tray_id=%u "
              "material=\"%s\" reason=unsupported_material",
              static_cast<unsigned>(command.printerId),
              static_cast<unsigned>(command.amsId),
              static_cast<unsigned>(command.trayId), command.trayType);
      publishBambuError(ctx, command.requestId, command.printerId,
                        "Material nicht unterst\xC3\xBCtzt, keine Bambu-Zuordnung hinterlegt");
      return;
    }
    const models::BambuMaterialRule& rule = *resolved.rule;
    std::snprintf(filament.trayInfoIdx, sizeof(filament.trayInfoIdx), "%s",
                 rule.result.trayInfoIdx);
    std::snprintf(filament.trayType, sizeof(filament.trayType), "%s",
                 rule.result.trayType);
    filament.nozzleTempMinC = rule.result.nozzleTempMinC;
    filament.nozzleTempMaxC = rule.result.nozzleTempMaxC;
    // "Fallback" here means the winning rule matched on material class
    // alone -- no name/manufacturer condition narrowed it to a more
    // specific Bambu profile (see docs/bambu-protocol.md).
    const bool isFallback = rule.match.nameContainsAny[0] == '\0' &&
                            rule.match.manufacturerExact[0] == '\0';
    FS_LOGD(services::LogComponent::Bambu,
            "[BAMBU] Material resolved material=\"%s\" name=\"%s\" "
            "manufacturer=\"%s\" rule=\"%s\" priority=%ld "
            "tray_info_idx=\"%s\" tray_type=\"%s\" temp_min=%u temp_max=%u "
            "fallback=%d",
            command.trayType, command.name, command.manufacturer, rule.id,
            static_cast<long>(rule.priority), rule.result.trayInfoIdx,
            rule.result.trayType,
            static_cast<unsigned>(rule.result.nozzleTempMinC),
            static_cast<unsigned>(rule.result.nozzleTempMaxC), isFallback);
  }
