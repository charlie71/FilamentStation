 ...
 *   LoadingSpool -> Failed    [label="LoadSpool failed, e.g. spool deleted"];
 * }
 * @enddot
 */
enum class TraySpoolDetailsStage : std::uint8_t {
  ...
  LoadingFilament,
  Loaded,
  // Terminal, deliberately NOT retried (Nutzerbericht 2026-08-31): the
  // LoadSpool step failed (e.g. the spool was deleted in Spoolman, HTTP
  // 404). Distinct from resetting back to Idle -- every subsequent
  // resolveTraySpoolDetails() call for the same tray (syncAmsToUi() runs
  // on every incoming Bambu MQTT status report, roughly every 1-2s) would
  // otherwise see an Idle-looking, now-spoolId-0 entry and immediately
  // re-issue the same doomed request, hammering Spoolman with 404s forever
  // and starving the SpoolmanTask command queue that a concurrently open
  // spool picker search also depends on.
  Failed,
};
...
// runtime-loaded, priority-ordered rule table (services::resolveBambuMaterialRule())
// keyed by material/name/manufacturer. The K-factor (flow_dynamics_k_factor)
...
struct PendingSlotAssignment {
  ...
  // name/manufacturer (Nutzerwunsch 2026-08-30, schema-v2 rule resolver):
  // let BambuTask disambiguate Bambu-specific profiles (e.g. "PLA Silk",
  // "Bambu PLA Basic") beyond the material class alone -- both already
  // available on the LoadSpool response, no extra request needed.
  char name[40]{};           ///< Spoolman filament product name resolved from the spool, sent to the printer.
  char manufacturer[32]{};   ///< Spoolman filament vendor name resolved from the spool, sent to the printer.
...
// captured material/name/manufacturer/colorHex (set when its LoadingSpool
// step completed). trayType carries the raw Spoolman material name (or
// empty, to clear the slot) -- BambuTask resolves the Bambu AMS profile
// (tray_info_idx/nozzle temperature range) from material/name/manufacturer
// via services::resolveBambuMaterialRule(), see docs/bambu-protocol.md; no
// Spoolman temperature is ever sent.
bool sendPendingSlotAssignTray(rtos::RtosContext& ctx, std::uint32_t requestId) {
  ...
  std::snprintf(assignTray.name, sizeof(assignTray.name), "%s",
               pendingSlotAssignment.name);
  std::snprintf(assignTray.manufacturer, sizeof(assignTray.manufacturer), "%s",
               pendingSlotAssignment.manufacturer);
...
void appTask(void* parameter) {
  ...
  for (;;) {
    ...
    if (event->type == rtos::AppEventType::UiAction) {
      ...
      // Material/Name/Hersteller/Farbe aus der aufgeloesten Spule; Bambu
      // erwartet trayColorHex als RRGGBB(AA)-Hexstring, colorHex[0] liefert
      // genau das. Keine Spoolman-Duesentemperatur mehr angefragt
      // (Nutzerwunsch 2026-08-27) -- BambuTask loest tray_info_idx/
      // Duesentemperatur per services::resolveBambuMaterialRule() auf
      // (Nutzerwunsch 2026-08-30: priorisierte Regeln statt reinem
      // Material-Lookup). Der K-Faktor ist dagegen eine spulenspezifische
      // Filament-Eigenschaft
      ...
      std::snprintf(pendingSlotAssignment.trayType,
                    ...
                    event->spool.material);
      std::snprintf(pendingSlotAssignment.name,
                    sizeof(pendingSlotAssignment.name), "%s",
                    event->spool.filament);
      std::snprintf(pendingSlotAssignment.manufacturer,
                    sizeof(pendingSlotAssignment.manufacturer), "%s",
                    event->spool.vendor);
      ...
      if (event->requestId >= kTraySpoolDetailsRequestIdBase &&
        ...
        if (entry.stage == TraySpoolDetailsStage::LoadingFilament) {
          ...
          // LoadSpool itself failed (e.g. the spool was deleted in
          // Spoolman, HTTP 404, Nutzerbericht 2026-08-31) -- mark this
          // spoolId as permanently Failed rather than resetting to `{}`
          // (which would clear spoolId back to 0 and make
          // resolveTraySpoolDetails() treat the very next sync, roughly
          // every 1-2s on every incoming Bambu MQTT report, as a fresh,
          // never-tried lookup and immediately retry the same doomed
          // request forever).
          const rtos::SpoolId failedSpoolId = entry.spoolId;
          entry = TraySpoolDetailsEntry{};
          entry.spoolId = failedSpoolId;
          entry.stage = TraySpoolDetailsStage::Failed;
        }
        continue;
      }
      if (currentScreen == rtos::UiScreenId::TagDefinitionImport ||
      ...
      if (pendingSlotAssignment.stage == SlotAssignmentStage::WritingSlot &&
        ...
        if (success) {
          ...
          // Duesentemperatur kommt jetzt immer aus dem priorisierten
          // Bambu-Material-Regelwerk (services::resolveBambuMaterialRule(),
          // BambuTask.cpp) --
