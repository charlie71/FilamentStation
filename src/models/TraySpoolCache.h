/**
 * @file
 * @brief Locally persisted printer/AMS/tray -> Spoolman-spool association
 *        cache (/mappings/printer-slots.json, see docs/storage.md).
 */
#pragma once
...
constexpr std::size_t kMaximumTraySpoolCacheEntries = 16;  ///< Maximum number of cached tray associations.
...
// spool holder.
constexpr std::uint8_t kExternalTraySentinel = 0xFF;  ///< amsId/trayId value pair used to key the external/manual spool holder's cache entry.

/// @brief One persisted printer/AMS/tray -> Spoolman-spool association,
///        plus the material/color the printer reported at assignment time.
///
...
struct TraySpoolCacheEntry {
  PrinterId printerId = kInvalidPrinterId;  ///< Printer this entry belongs to.
  std::uint8_t amsId = 0;                   ///< 0-based AMS index, or kExternalTraySentinel.
  std::uint8_t trayId = 0;                  ///< Tray index within the AMS unit, or kExternalTraySentinel.
  std::uint32_t spoolId = 0;                ///< Associated Spoolman spool id.
  ...
  char material[16]{};  ///< Material reported by the printer at assignment time (fingerprint).
  char colorHex[9]{};   ///< Color reported by the printer at assignment time (fingerprint); may be empty if the spool has no configured color.
};

/// @brief Fixed-capacity collection of TraySpoolCacheEntry values, the
///        in-memory mirror of /mappings/printer-slots.json.
struct TraySpoolCache {
  std::array<TraySpoolCacheEntry, kMaximumTraySpoolCacheEntries> entries{};  ///< Backing storage; only the first #entryCount entries are valid.
  std::uint8_t entryCount = 0;  ///< Number of valid entries in #entries.
};

/// @brief Finds a cached association by its (printer, AMS, tray) key.
/// @param cache Cache to search.
/// @param printerId Printer id.
/// @param amsId AMS index, or kExternalTraySentinel.
/// @param trayId Tray index, or kExternalTraySentinel.
/// @return Pointer to the matching entry, or nullptr if none exists.
...
/// @brief Const overload of findTraySpoolCacheEntry().
/// @param cache Cache to search.
/// @param printerId Printer id.
/// @param amsId AMS index, or kExternalTraySentinel.
/// @param trayId Tray index, or kExternalTraySentinel.
...
/// @brief Inserts or overwrites the entry for (printerId, amsId, trayId).
/// @param cache Cache to modify.
/// @param value Entry to insert/overwrite (its printerId/amsId/trayId form the key).
/// @return False only if the cache is full and no matching entry already existed.
...
/// @brief Removes the entry for (printerId, amsId, trayId), if any.
/// @param cache Cache to modify.
/// @param printerId Printer id.
/// @param amsId AMS index, or kExternalTraySentinel.
/// @param trayId Tray index, or kExternalTraySentinel.
