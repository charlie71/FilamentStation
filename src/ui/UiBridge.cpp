...
void showScreen(rtos::UiScreenId screenId) {
  switch (screenId) {
    ...
    case rtos::UiScreenId::SettingsDiagnostics: {
      char text[72];
      std::snprintf(text, sizeof(text), "Heap: %lu kB frei (min. %lu kB)",
                    static_cast<unsigned long>(ESP.getFreeHeap()>>10),
                    static_cast<unsigned long>(ESP.getMinFreeHeap()>>10));
      lv_label_set_text(objects.diagnostics_settings_heap, text);
      std::snprintf(text, sizeof(text), "PSRAM: %lu kB frei (min. %lu kB)",
