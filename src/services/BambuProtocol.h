...
///        bambuBuildAmsFilamentSetting(). Resolved from a Spoolman filament
///        via services::resolveBambuMaterialRule() (see
///        services/BambuMaterialCatalog.h) -- this file has no knowledge of
///        the resolution rules themselves, only the resulting wire fields.
struct BambuTrayFilament {
  char trayInfoIdx[16]{};           ///< Bambu generic filament profile id ("tray_info_idx" on the wire).
  ...
  std::uint16_t nozzleTempMinC = 0;  ///< Minimum nozzle temperature.
  std::uint16_t nozzleTempMaxC = 0;  ///< Maximum nozzle temperature.
};
...
///        caller resolves it via services::resolveBambuMaterialRule()); no "setting_id"
...
/// @param trayInfoIdx Generic filament profile id, echoed back as "filament_id" (see services::resolveBambuMaterialRule()).
