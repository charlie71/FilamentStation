...
struct BambuCommand {
  ...
  // BambuTask resolves material/name/manufacturer to a Bambu AMS profile
  // itself via services::resolveBambuMaterialRule() (schema-v2 rule table,
  // Nutzerwunsch 2026-08-30); no temperature field here, the wire
  // nozzle_temp_min/max always come from that resolution, never from
  // Spoolman. `name`/`manufacturer` are only used to disambiguate Bambu-
  // specific profiles (e.g. "PLA Silk", "Bambu PLA Basic") -- left empty,
  // resolution falls back to whatever rule matches `trayType` alone.
  char trayType[16]{};              ///< Material to write, for AssignTray (empty clears the slot).
  char name[40]{};                  ///< Spoolman filament product name, for AssignTray (see models::SpoolmanSpool::filament).
  char manufacturer[32]{};          ///< Spoolman filament vendor name, for AssignTray (see models::SpoolmanSpool::vendor).
