...
## Nachtrag 2026-08-30 (7): Spoolman → Bambu Material Resolver auf regelbasiertes Schema v2 umgestellt

Größere Projektänderung (Auftragsbeschreibung): der bisherige flache
Materialkatalog (Schema v1: `material` → genau ein Bambu-Profil) kann
Bambu-eigene Spezialprofile (z. B. "Generic PLA Silk", "Generic PLA High
Speed", "Bambu PLA Basic") nicht abbilden, da Spoolman diese
Unterscheidung nicht im `material`-Feld trägt, sondern im
Filament-`name`/Hersteller. Ersetzt durch ein priorisiertes Regelwerk.
Ausführliche Protokoll-/Schema-Dokumentation in `docs/bambu-protocol.md`
(neuer Nachtrag 2026-08-30) und `data/bambu-materials/README.md`
(komplett überarbeitet); hier nur die Zusammenfassung.

**Geänderte Dateien:** `src/models/BambuMaterialMapping.h` (neues
Regel-Datenmodell), `src/services/BambuMaterialCatalog.{h,cpp}` (Parser
für Schema v2 + neuer Resolver, ersetzt den alten Schema-v1-Parser),
`src/services/BambuProtocol.{h,cpp}` (alte `resolveBambuMaterial()`/
`sameMaterialKey()` entfernt, gehören jetzt in BambuMaterialCatalog),
`src/tasks/BambuTask.cpp` (`handleAssignTray()` nutzt den neuen
Resolver + erweiterte Fehlerbehandlung/Logging), `src/tasks/
AppTask.cpp` (`PendingSlotAssignment`/`sendPendingSlotAssignTray()`
tragen jetzt zusätzlich `name`/`manufacturer`), `src/tasks/
StorageTask.cpp` (Typnamen/Log-Felder an Schema v2 angepasst,
Download-/SHA-256-/Aktivierungsinfrastruktur **unverändert**
wiederverwendet), `src/rtos/Messages.h` (`BambuCommand` um `name`/
`manufacturer` erweitert), `src/rtos/RtosContext.h` (Typname +
aktualisierte Größenangabe), `src/config/BambuMaterialConfig.h`
(`kBambuMaterialsMaxFileSize` 16→48 KiB), `src/services/
SpoolmanCatalog.cpp` (Kommentarverweis aktualisiert),
`data/bambu-materials/bambu_materials.json(.sha256)` + Root-Kopien
(migrierter Katalog + neue Prüfsumme), `data/bambu-materials/README.md`,
`docs/bambu-protocol.md`, `test/test_bambu_material_catalog/test_main.cpp`
(komplett neu für Schema v2), `test/test_bambu_protocol/test_main.cpp`
(alte `resolveBambuMaterial`-Tests entfernt, Funktion existiert nicht
mehr in diesem Modul).

**Neue/angepasste Datenstrukturen:** `models::BambuMaterialRule`
(`id`, `priority`, `match`, `result`), `models::BambuMaterialMatch`
(`materialExact`/`nameContainsAny`/`manufacturerExact`, je ein
'|'-separierter Puffer -- gleiches Speicherprinzip wie das alte
Aliase-Feld, keine 2D-Arrays), `models::BambuMaterialRuleResult`
(`status`, `trayInfoIdx`, `trayType`, `nozzleTempMinC/MaxC`, `reason`),
`models::BambuMaterialRuleTable` (ersetzt `BambuMaterialMappingTable`),
`services::BambuMaterialResolveInput` (`material`/`name`/
`manufacturer`, reine `const char*`, keine `std::string_view` --
passend zum bestehenden C-String-Stil des Projekts),
`services::BambuMaterialResolveResult` (`status`
`Mapped`/`Unsupported`/`NoMatch`/`Ambiguous`, `rule`, `matchedPriority`,
`ambiguousRuleIds`).

**Katalog-Umfang:** 77 Regeln gesamt -- 58 `mapped` (40 migrierte
Schema-v1-Basismaterialien + 14 migrierte Supportmaterialien + 4 neue
Spezialprofile: PLA Silk/GFL96, PLA High Speed/GFL95, PETG HF/GFG96,
Bambu PLA Basic/GFA00 [nur bei `manufacturer_exact: "Bambu Lab"`]),
19 `unsupported` (Wood, Carbon Fiber, PC/ABS, PC/PBT, Acetal (POM),
PMMA, Semi flexible (FPE), PVDF, PEI (Ultem), PEKK, PEEK, PPSU,
BIOFUSION, GREENTEC, FLAX, PEARL, Flexible (TPE 32D), Flexible
(TPE 88A), PVB -- jeweils mit Begründung, kein erfundener Bambu-Code).
Alle 33 aktuellen SpoolmanDB-Materialtypen bewusst behandelt (14
mapped + 19 unsupported); `PLA+`/`ABS+`/`ABS-T`/`Nylon`/
`Flexible (TPU)`/`Polycarbonate (PC)`/`Polypropylene (PP)` als
zusätzliche `material_exact`-Werte in die jeweils passende bestehende
Basisregel eingepflegt statt als eigene Regeln.

**Zusätzlich verwendete Spoolman-Felder:** `name` (Filament-Produktname,
bereits vorhanden als `models::SpoolmanSpool::filament`) und
`manufacturer` (`models::SpoolmanSpool::vendor`) -- beide waren schon
Teil der bestehenden `LoadSpool`-Antwort (`SpoolmanTask.cpp::
parseSpool()`) und wurden bisher nur nie weitergereicht; kein
zusätzlicher HTTP-Request, kein zweites paralleles Spoolman-Datenmodell.

**Tests:** 115/115 native Tests grün (vorher 105 -- 26 alte
`resolveBambuMaterial`/Schema-v1-Tests entfernt, 36 neue
Parser-/Resolver-Tests für Schema v2 ergänzt, davon 3 laden die **echte**
Repository-Datei und prüfen alle 33 SpoolmanDB-Typen sowie alle
Spezial-/Support-Regeln gegen -- fängt künftig eine kaputte/mehrdeutige
Katalog-Änderung bereits im nativen Testlauf ab). Build 0 Warnungen.

**Neue SHA-256-Prüfsumme:**
`6e430b84980c0c7f3179b06c7605360a5c30d603fbb85dc385a9488ed3fc7ecd`
(sowohl `bambu_materials.json.sha256` als auch
`data/bambu-materials/bambu_materials.json.sha256`).

**Noch nicht eindeutig zuordenbar / offene Verifikationspunkte:**
(1) die 19 als `unsupported` markierten SpoolmanDB-Typen -- absichtlich,
kein Bambu-Profil dafür verifiziert; (2) neu entdeckt bei der Migration:
der bereits vorher existierende Schema-v1-Eintrag `TPU 95A` zeigt auf
denselben `tray_info_idx` (`GFL95`), den diese Aufgabe jetzt explizit
für "Generic PLA High Speed" vorgibt -- beide Zuordnungen stammen aus
unterschiedlichen, unverifizierten Community-Quellen; mindestens eine
dürfte falsch sein (siehe `docs/bambu-protocol.md`). **Kein
Hardware-Test möglich** -- insbesondere die neuen `tray_info_idx`-Werte
(`GFL95`/`GFL96`/`GFG96`/`GFA00`) sind wie der gesamte bisherige Katalog
unverifiziert gegen echte Bambu-Studio-Profile.

## Nachtrag 2026-08-31: Endlose 404-Retry-Schleife für gelöschte Spulen behoben

Nutzerbericht: Tray 1 hat laut Cache Spoolman-Spule #11, die im Spoolman
inzwischen gelöscht wurde. `resolveTraySpoolDetails()` (Home-Tray-Karten-
Anreicherung mit Restgewicht/K-Faktor) versuchte diese Spule dauerhaft
alle ~1-2 Sekunden neu zu laden -- ausgelöst von jedem eingehenden
Bambu-MQTT-Statusbericht (`syncAmsToUi()`), der `resolveTraySpoolDetails()`
für jedes belegte Fach erneut aufruft. Ursache: der Fehlerpfad setzte den
Cache-Eintrag bei einem fehlgeschlagenen `LoadSpool` bisher komplett
zurück (`entry = TraySpoolDetailsEntry{}`), wodurch auch `spoolId` auf 0
fiel -- der nächste Aufruf fand dadurch keinen "bereits versucht"-Eintrag
mehr und startete denselben aussichtslosen Request erneut, ein Zyklus
ohne Ende (Spoolman bekam alle ~2s ein HTTP-404 serviert).

Fix: neue, terminale `TraySpoolDetailsStage::Failed` -- der Eintrag wird
bei einem `LoadSpool`-Fehlschlag jetzt mit unverändertem `spoolId`, aber
`stage = Failed` markiert statt komplett geleert. `resolveTraySpoolDetails()`s
bestehende Guard-Logik (`stage != Idle` → keine Neuanfrage) greift dadurch
automatisch auch hier, ohne eigene Änderung an dieser Funktion nötig.
Verhalten wie bei einer noch nicht geladenen Spule (`loaded=false`, kein
Absturz) -- nur eben dauerhaft, bis der Cache-Eintrag durch eine andere
Spule verdrängt wird oder das Gerät neu startet (gleiche
"nie neu laden, reines Anzeige-Snapshot"-Philosophie wie beim
`Loaded`-Zustand).

**Zum ebenfalls berichteten Symptom** ("Spulen auswählen"-Dialog auf
SCR_STAGING_ACTIONS schließt sich nach ca. 3 s von selbst): den
kompletten Pfad nachvollzogen -- `SpoolmanTask` verarbeitet Kommandos
strikt sequenziell (`xQueueReceive(..., portMAX_DELAY)`, ein Request nach
dem anderen, kein Überlappen möglich) und jede der geloggten
404-Anfragen für Spule 11 war in ~12-20 ms fertig; das reicht bei
Weitem nicht, um eine gleichzeitige Sucheanfrage des Pickers spürbar zu
verzögern. Auch keine generische Overlay-Timeout-/Auto-Close-Logik in
`UiBridge.cpp`/`AppTask.cpp` gefunden, die dafür infrage käme. Die
Retry-Schleife ist also mit hoher Wahrscheinlichkeit **nicht** die
direkte Ursache des Dialog-Schließens, auch wenn beide Symptome zeitlich
zusammen auftraten -- das jetzt behobene Problem ist trotzdem ein
eigenständiger, echter Fehler. Für das Dialog-Symptom fehlt bislang ein
Log, das den Moment des Schließens selbst abdeckt (Sucheanfrage/-antwort,
evtl. Fehlerdialog) -- ohne das nur eine weitere Vermutung, kein
begründeter Fix.
